...
> **Status: raw idea only — but comprehensive.** This document describes the *concept* in full: the problem, the users, the experience, the behavior, the principles, and the vision. It deliberately makes **no implementation choices** — no language, framework, architecture, or specific tooling is implied or preferred. The only fixed constraint is the platform (below). Everything about *how* it gets built is open.
